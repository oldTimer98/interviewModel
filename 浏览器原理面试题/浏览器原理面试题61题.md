...
**缓存查找优先级链路：** Memory Cache → Service Worker → Disk Cache → 网络请求

- **Memory Cache** 速度最快（0ms），但容量小，页面关闭即失效
- **Service Worker** 可自定义缓存策略（约10ms），断网可用，不受 disable cache 影响
- **Disk Cache** 容量最大，持久存储，跨站点相同资源可复用
- **无缓存** 走网络请求（约100ms+）
...
| 属性 | 作用 |
| --- | --- |
| `Max-Age` / `Expires` | 控制 Cookie 有效期 |
| `Secure` | 仅在 HTTPS 环境中发送 |
| `HttpOnly` | 禁止 JS 通过 `document.cookie` 读取，防止 XSS 窃取 |
| `Domain` | 指定 Cookie 可送达的域名 |
| `Path` | 限制 Cookie 生效路径 |
| `SameSite` | 防止 CSRF，三个值：`Strict`（仅同源）、`Lax`（允许链接跳转的 GET）、`None`（不限制，但需同时设 Secure） |

**Cookie + Session 登录态流程：**

1. 用户登录后，服务端生成唯一 `session_id`
2. 服务端通过 `Set-Cookie` 下发 `session_id` 到客户端
3. 后续请求浏览器自动带上 Cookie，服务端通过 `session_id` 映射到用户信息

**JWT 与 Cookie-Session 对比：**

| 特性 | Cookie-Session | JWT |
| --- | --- | --- |
| 状态存储 | 服务端存储 session | 无状态，token 自包含用户信息 |
| 分布式支持 | 需共享 session（Redis 等） | 天然支持分布式 |
| 适用场景 | 浏览器环境 | 浏览器 + App + 跨域 |
| 安全性 | 需防 CSRF | 需防 XSS（token 存 localStorage） |
| 主动注销 | 服务端删 session 即可 | 需额外机制（黑名单等） |
...
- **Access-Control-Allow-Origin** 设置为非 `*`（需指定具体域名）
...
常见的创建合成层的方式：`transform`、`opacity`、`filter`。利用这些属性可以减少重排重绘，提升动画性能。

**will-change 属性：** 告知浏览器该元素会有哪些变化，让浏览器提前做好优化准备：
...
**使用注意事项：**
- 不要将 will-change 应用到太多元素上，过度使用可能消耗大量内存
- 不要过早应用，应在元素变化前通过脚本动态添加，变化后移除
- 在低端移动设备上需谨慎使用，可能导致图片模糊等问题
- 两种强制开启硬件加速的方式：`transform: translateZ(0)` 和 `will-change: transform`
...
WebSocket 是 HTML5 引入的全双工通讯协议，连接建立后服务端和客户端都可以主动向对方发送消息。
...
| 开销 | 每次都需 HTTP 头，资源浪费 | 连接建立后开销极小 |
| 实时性 | 取决于轮询间隔 | 即时 |
| 连接 | 短连接，频繁建立/断开 | 长连接，一次握手持续通信 |

**握手过程（复用 HTTP，101 协议升级）：**
...
**核心特性：**
- **数据分片**：消息被拆分为多个帧，通过 FIN 标志判断是否发送完毕
- **心跳机制**：ping/pong 帧保持连接活跃，检测断连
- **支持跨域**：不受同源策略限制
- **不携带 Cookie**：需自行维护身份认证

**应用场景：** 在线聊天/IM、实时协同编辑、Web 游戏、webpack-dev-server 热更新、股票行情推送

**最佳实践：**
- 使用 wss（加密的 WebSocket）
- 做好断线重连逻辑
- 使用成熟库如 socket.io（支持降级兼容）

## 57.为什么浏览器会出现白屏？如何检测与预防？

页面因某些原因无法正常渲染，导致用户看到空白页面的现象。

**白屏类型：**

| 类型 | 说明 |
| --- | --- |
| 纯白屏 | 页面从加载到结束一直白屏，无任何响应 |
| Loading 白屏 | 出现了 Loading 动画但内容始终不出来 |
| 局部白屏 | 页面部分区域因异常导致元素缺失 |

**常见原因：**
- 关键静态资源 404（HTML/JS 文件，SPA 场景尤其致命）
- JS 未捕获异常：`undefined.xxx`、`JSON.parse(非法字符串)`
- 死循环导致页面卡死
- 框架组件异常导致整体 crash（React/Vue 单组件错误未被 ErrorBoundary 捕获）

**检测方案：**

| 方案 | 原理 | 适用场景 |
| --- | --- | --- |
| 采样检测 | `elementsFromPoint` 检测多个坐标点是否都是空容器 | 纯白屏 |
| DOM 打分 | 广度遍历 DOM，按层级计算可见性分数 | 白屏 + 骨架屏 + 局部白屏 |
| Service Worker 心跳 | SW 独立线程监控页面心跳，超时即判定崩溃 | 页面 crash |

**预防措施：**
- 不信任上游数据，做好输入校验
- React 使用 ErrorBoundary，Vue 使用 `errorHandler`
- 全局异常兜底：`window.onerror` + `unhandledrejection` + `addEventListener('error', fn, true)`
- 跨域脚本加 `crossorigin="anonymous"`，服务端返回 `Access-Control-Allow-Origin`

## 58.HTTP 从 1.0 到 3.0 的演化之路

**速记口诀：** 1.0 短连接 → 1.1 长连接 → 2.0 多路复用 → 3.0 QUIC/UDP

| 版本 | 核心改进 | 解决的问题 |
| --- | --- | --- |
| HTTP/1.0 | 引入状态码、Header、Content-Type | 只能传 HTML → 支持多种内容 |
| HTTP/1.1 | keep-alive 长连接、管道化、分块传输、Host 头 | 每次请求都要新建 TCP → 连接复用 |
| HTTPS | TLS 加密（非对称交换密钥 + 对称传输数据） | 明文传输不安全 → 加密 + 身份验证 |
| HTTP/2 | 二进制分帧、多路复用、HPACK 头压缩、Server Push | 队头阻塞、头部冗余、无法推送 |
| HTTP/3 | 基于 UDP 的 QUIC 协议、连接迁移 | TCP 队头阻塞、握手延迟、网络切换断连 |

**HTTP/2 关键特性：**
- **二进制分帧**：将数据拆分为帧（Frame），帧属于流（Stream），多个流复用一个 TCP 连接
- **多路复用**：一个连接上并发多个请求/响应，不再需要多域名突破并发限制
- **HPACK 头压缩**：静态表 + 动态表 + 霍夫曼编码，大幅减小 Header 体积
- **Server Push**：服务端主动推送客户端将要请求的资源

**HTTP/3 (QUIC) 关键改进：**
- **0-RTT 建连**：UDP 无需三次握手，首次连接 1-RTT，后续 0-RTT
- **无队头阻塞**：每个 stream 独立，单个 stream 丢包不影响其他 stream
- **连接迁移**：使用 Connection ID 标识连接，切换 WiFi/4G 不断连

## 59.如何实现浏览器指纹（设备指纹）？

浏览器指纹是通过收集浏览器特征信息来唯一标识设备或用户的技术，即使未登录或使用无痕模式也能识别。

**三类指纹：**

| 类型 | 原理 | 示例 |
| --- | --- | --- |
| 基本指纹 | 收集浏览器/系统特征 | UA、屏幕分辨率、语言、时区、插件列表 |
| Canvas 指纹 | 不同系统/浏览器渲染 Canvas 结果不同 | 绘制文本/图形 → toDataURL → Hash |
| 音频指纹 | 不同硬件/软件处理音频信号的差异 | 生成音频流 → FFT/压缩 → Hash |
...
**组合指纹**：单项信息熵有限，但将基本 + Canvas + 音频指纹组合后几乎可以唯一标识。

**应用场景：** 精准广告推送、反黑产（识别同设备多账号水军）、风控系统

**防范手段：**
- HTTP 头 `DNT: 1`（Do Not Track，但多数网站未遵守）
- 禁用 JS（暴力但影响功能）
- 使用 Tor 浏览器等隐私工具
- 即使禁用 JS，CSS `@media` 查询仍可泄露屏幕分辨率等信息
...
| | 矢量图 | 位图（光栅图） |
| --- | --- | --- |
| 原理 | 用数学公式描述线、点、多边形 | 逐像素存储颜色值 |
| 缩放 | 不失真 | 放大后产生锯齿 |
| 适用 | 图标、Logo、几何图形 | 照片、复杂场景 |
| 格式 | SVG、CSS3 | JPEG、PNG、GIF、WebP、AVIF |
...
| PNG-8 | ✅ | ❌ | 无损 | 256色 | 图标、简单图形 |
| PNG-24 | ✅ | ❌ | 无损 | 直接色 | 需要透明的高质量图 |
| JPEG | ❌ | ❌ | 有损 | 直接色 | 照片、颜色丰富的图 |
| WebP | ✅ | ✅ | 有损+无损 | 直接色 | 替代 JPEG/PNG（兼容性 95%+） |
| AVIF | ✅ | ✅ | 有损+无损 | 直接色 | 有损压缩最优，照片/广告 |
| JPEG XL | ✅ | ✅ | 有损+无损 | 直接色 | 无损压缩最优，可无损转旧 JPEG |

**选型建议：**
- 照片/广告 → AVIF（有损最优）> WebP > JPEG
- UI 设计/存档 → JPEG XL（无损最优）> PNG
- 简单动画 → WebP > GIF
- 图标/Logo → SVG（矢量）

## 61.CDN 与资源交付优化

**CDN（内容分发网络）** 通过在全球部署分布式代理服务器，让用户从最近的节点获取资源。

**CDN 核心原理：**
1. 用户请求 → DNS 解析 → 智能调度到最近的 CDN 节点
2. CDN 节点有缓存 → 直接返回（命中）
3. CDN 节点无缓存 → 回源到源站获取 → 缓存后返回

**关键术语：**
- **回源**：CDN 节点缓存未命中，向源站请求资源
- **缓存**：将源站资源按策略存储在 CDN 节点
- **边缘节点**：离用户最近的 CDN 服务器

**CDN 的性能优势：**
- **物理距离更短**：减少 RTT 延迟
- **网络优化**：CDN 服务商优化了骨干网络链路
- **缓存命中**：无需回源，直接从边缘节点响应
- **减轻源站压力**：适合处理流量峰值
- **抗 DDoS**：分布式节点分散攻击流量
...
- 使用多域名突破浏览器并发连接限制（HTTP/1.1 时代）
- 配合 `preconnect` 预连接 CDN 域名
- 资源 URL 添加版本号/hash 实现缓存更新
