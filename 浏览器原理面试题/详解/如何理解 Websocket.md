...
HTML5 引入的，用于实现客户端-服务器之间全双工通讯的网络协议，一旦连接成功，后续服务端、客户端都可以主动向对方发送消息
...
- 复用 http 协议，借助 101 响应码实现协议升级(**复用 http、https 默认端口**)
...
- WS 握手过程强依赖于 http，可复用 80/443 端口、复用网络基础设施等
- 全双工通讯 vs 服务端推送？这不是一个东西

- 打电话：ws 中，server 可以随时通过 ws 下发消息，这些消息也会立即触发客户端对应回调 —— 即时处理
- 发电报：服务端推送，则更像在客户端请求某个具体资源之前，现行下发资源数据

- 二进制分帧？

- 这个倒是真有点像，都是将消息拆解为多个分片，都是将分片内容包装成一个个特定数据帧格式，设计思维很像；主要差异点：

- 两者帧协议不同，例如：二进制分帧通过数据帧的 END_STREAM 位标识消息体已经发送完毕；ws 则通过 FIN 位标识单个消息发送完毕

- 也都支持并发

- 有状态，所以需要注意保存 ws 实例、处理 cookie、处理重连逻辑等
- 适合处理需要频繁交互的场景，但在普通的 r-r 请求作用不大