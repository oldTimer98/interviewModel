...
总结：<keep-alive> 是 Vue 的内置抽象组件，用于缓存动态组件实例，避免重复销毁/创建。
它通过拦截组件的生命周期，在 deactivated 时将组件的 VNode 和实例存入一个 LRU 缓存对象，而不是调用 destroyed；激活时从缓存取出并触发 activated。
缓存的内容包括组件实例（data、methods）、真实 DOM 节点及其子树状态。
使用时要注意：配合 include/exclude 控制范围，设置 max 防止内存泄漏，并在 deactivated 中清理定时器或全局事件
