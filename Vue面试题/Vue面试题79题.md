...
Vue 3 在**编译阶段**对静态标记做了重大升级（注意：这是编译器优化，与 Proxy 响应式无关）：

1. **静态节点提升（hoistStatic）**：编译器将不会变化的静态节点提取到渲染函数外部，作为模块级常量，组件更新时直接复用，避免重复创建 VNode。
2. **patchFlag 标记**：编译器为动态节点添加 patchFlag（如 `1 /* TEXT */`、`2 /* CLASS */`），告诉运行时 diff 只需对比标记的部分，跳过静态属性。
3. **Block Tree**：将模板中的动态部分（v-if、v-for 等）组织为 Block，diff 时只遍历 Block 内的动态节点，而非整个子树。
...
3. 生命周期改变：Vue3废除了Vue2中的beforeDestroy和Destroyed钩子函数，并新增了两个钩子函数：beforeUnmount 和unmounted；此外，activated和deactivated在Vue3中仍然可用（Options API），Composition API 中对应 onActivated 和 onDeactivated。
4. 异步组件改变：Vue3使用全局函数defineAsyncComponent来定义异步组件，从而方便了异步组件的使用。
5. Teleport改变：Vue3新增Teleport组件，通过 `to` prop 指定渲染目标位置，支持 `disabled` prop 控制是否传送。
...
在 Vue 2 中，组件只能有**一个根节点**，Vue 3 中组件可以返回**多个根节点**，这称为 Fragment：
...
允许将子节点渲染到 DOM 树的任何位置，而不仅仅是父组件内部。常用于模态框、提示信息、弹出菜单等：
...
**性能优势**：将模态框等 UI 元素移到 body 下，减少父容器的重绘和回流。

#### Suspense（异步等待）

用于处理异步组件的加载状态，提供 fallback 内容：
...
借助 Suspense 可以实现：懒加载组件减少初始加载时间、统一的加载状态管理、更优雅的错误处理。

#### 自定义渲染器 API

Vue 3 提供了创建自定义渲染器的能力，使 Vue 不仅限于浏览器，还可用于小程序、原生应用等环境。
...
| 优化手段 | 原理 | 效果 |
| --- | --- | --- |
| **静态树提升** | 编译时标记静态 VNode，提升为常量，不参与 diff | 减少每次更新的 VNode 创建 |
| **按块跟踪** | 将动态内容分为独立 Block，只更新变化的块 | 减少不必要的子树对比 |
| **Proxy 响应式** | 惰性代理，按需拦截 | 减少初始化成本，支持 Map/Set |
| **Tree-shaking** | ESM 按需导入 | 减小打包体积 |
| **函数式插槽** | 插槽内容延迟渲染，独立更新 | 避免父组件更新引发子插槽重渲染 |

**静态树提升示例**：静态节点编译后提升为模块级常量，组件更新时直接复用：
...
function render(_ctx) {
  return openBlock(), createBlock("div", null, [
    _hoisted_1,
    _hoisted_2,
    createVNode("div", null, toDisplayString(_ctx.dynamicContent), 1)
  ])
}
```
...
Vue的**响应式数据绑定机制**是Vue最核心的特性之一。通过对数据进行劫持和监听，可以实现数据的双向绑定，即数据变化会自动更新视图，同时视图的变化也会反映到数据上。这种机制使得Vue的数据流非常清晰和可预测，同时也减少了开发的工作量。
...
- **运行速度更快：相比较于 react而言，同样是操作虚拟 dom，Vue 默认按依赖追踪做组件级更新，React 需手动优化（memo/shouldComponentUpdate），各有策略。**
...
Watcher对象通过发布/订阅模式实现，即Watcher对象订阅响应式数据对象的变化，响应式数据对象在变化时发布通知，通知订阅者(Watcher>对象)进行更新。
...
注意：Object.defineProperty() 本身 可以 监听到数组的“下标（索引）变化”，但前提是：你提前为每个索引定义了 getter/setter
...
1. 不支持低版本浏览器。
2. 代理的是整个对象而非单个属性，需要通过 handler 的 get/set 在内部按属性处理。
...
**实现原理**：当子组件vm实例化时，获取到父组件传入的slot标签的内容，存放在 vm.$slot中，默认插槽为 vm.$slots.default，具名插槽为 vm.$slots.xxx，xxx 为插槽名，当组件执行渲染函数时候，遇到slot标签，使用 $slot中的内容进行替换，此时可以为插槽传递数据，若存在数据，则可称该插槽为作用域插槽。
...
只需要在组件即将被销毁的生命周期 beforeUnmount（Vue3）或 beforeDestroy（Vue2）中在 LocalStorage / SessionStorage 中把当前组件的 state 通过 JSON.stringify() 储存下来就可以了。在这里面需要注意的是组件更新状态的时机。
...
**（2）路由传值**
通过 Vue Router 的路由跳转可以实现路由间传递参数的效果。
在这里可以用 params 或 query 参数，在目标组件中通过 this.$route.params 或 this.$route.query 就可以拿到参数值，保存它。返回 A 组件时再次携带 state 达到路由状态保持的效果。
...
### 深入源码：v-show 与 v-if 的实现差异

#### v-show 原理（vue-next 源码）

v-show 本质是一个指令，通过 CSS `display` 属性控制元素显隐：

```javascript
// vue-next/packages/runtime-dom/src/directives/vShow.ts
export const vShow = {
...
v-if 在编译阶段被转换为条件表达式，通过 render 函数决定是否生成 VNode：

```javascript
// compiler-core/src/transforms/vIf.ts
...
**关键区别总结**：v-show 在运行时通过指令钩子切换 CSS，DOM 元素始终存在；v-if 在编译时转换为条件分支，false 时不创建 VNode，DOM 节点不存在。
...
在这个例子中，`v-model` 将 `input` 元素的值与 `message` 数据属性绑定，这样每当表单元素的值改变时，数据对象的属性值都会同步更新。
...
### 从编译看不同表单元素的 v-model 实现

#### input type="text" 编译结果

```javascript
// <input type="text" v-model="foo">
_c('input', { 
  directives: [{ name: "model", value: (foo), expression: "foo" }], 
  attrs: { "type": "text" }, 
  domProps: { "value": (foo) }, 
  on: { 
...
_c('input', { 
  ...
  on: { 
    "change": function ($event) { 
      var $$el = $event.target, $$c = $$el.checked ? true : false
      if (Array.isArray(bar)) { 
        var $$v = null, $$i = _i(bar, $$v)
        if ($$el.checked) { $$i < 0 && (bar = bar.concat([$$v])) } 
        else { $$i > -1 && (bar = bar.slice(0, $$i).concat(bar.slice($$i + 1))) } 
      } else { bar = $$c } 
    } 
  } 
})
...
| 对比项 | `v-model` | `.sync`（Vue2） |
| --- | --- | --- |
| 本质 | `:value` + `@input` | `:propName` + `@update:propName` |
| 数量限制 | Vue2 只能用一个，Vue3 支持多个 | 可以同时用多个 |
| 使用场景 | 表单双向绑定、自定义组件主值 | 父子组件属性同步 |
| Vue3 变化 | `v-model:propName` 统一了两者 | 已废弃，被 v-model 取代 |
...
// Vue3 中统一为 v-model
<son v-model:a="num" v-model:b="num2"></son>
```
...
Vue.nextTick()方法的实现原理是基于浏览器的异步任务队列，采用微任务优先的方式。当我们修改数据时，Vue.js会将DOM更新操作放到一个异步任务队列中，等待下一次事件循环时执行。而Vue.nextTick方法则是将一个回调函数推入到异步任务队列中，等待DOM更新完成后执行。
...
但是，由于Vue.js的异步更新机制，当我们修改数据后，如果想要立即获取更新后的DOM,可能会出现获取到的是更新前的DOM的情况。这时就需要使用Vue.nextTick方法。
...
首先，$nextTick 方法拥有一个异步队列，用于存放所有的回调函数。当第一个回调函数被添加进来时，就会创建一个异步的微任务，并且记录任务的 ID。接下来，如果有其他回调函数添加到队列中，就会直接向队列中添加回调函数，而不是再创建一个微任务。

当微任务执行时，会执行所有保存在队列中的回调函数，并在执行之后，清空所有的队列。
...
以上代码中，我们创建了一个 callbacks 数组来保存所有的回调函数，同时设置一个 pending 标志位来表示异步队列当前状态是否被 pend，如果 pend 为 false，则表示当前异步队列中已没有任务执行，这时就会创建一个 Promise 实例（微任务），并在 then 方法中执行 flushCallbacks 方法。当 flushCallbacks 被调用时，会将 pending 标志位设为 false，并使用 Array.slice() 方法来深拷贝 callbacks 数组，以遍历拷贝后的数组并执行所有的回调函数。最后再将 callbacks 数组清空以便保存下一个异步队列任务。
...
**记忆口诀：「异步队列批更新，同 watcher 只推一次，nextTick 再刷 DOM」**
...
**记忆口诀：「新属性非响应式，Vue2 用 $set，Vue3 Proxy 自动追踪」**
...
**记忆口诀：「Vue2: bind/inserted/update/componentUpdated/unbind；Vue3: created/mounted/updated/unmounted」**
...
使用自定义指令后，setup 函数中的组件渲染逻辑会被包裹进 `withDirectives` 函数：
  ...
  setup(__props) {
    return (_ctx, _cache) => {
      ...
      return withDirectives(
        (openBlock(), createBlock("div", null, [_hoisted_1], 512)),
...
- `withDirectives`：给 VNode 挂载 `dirs` 属性，记录该节点的所有指令调用
- 渲染过程中，在不同阶段触发对应的指令生命周期钩子
...
| --- | --- | --- |
| `bind` | `created` | 元素创建后 |
| `inserted` | `mounted` | 插入 DOM 后 |
| `update` | `beforeUpdate` | 更新前 |
| `componentUpdated` | `updated` | 更新后 |
| `unbind` | `unmounted` | 解绑时 |

#### 设计意图（对比 React）

- Vue 的 template 是 HTML 的超集，通过自定义指令补充 `v-for`、`v-if` 等声明式能力；而 JSX 是 JS 的超集，天然具备 if/for 等控制流
- Vue 用指令将 DOM 操作逻辑收敛到同一个地方，高内聚低耦合；React 用 ref 直接给出节点引用，声明逻辑与 DOM 操作混在一起
- Vue3 不推荐在自定义组件上使用指令（多根节点时指令失效），指令中也不推荐直接修改组件状态


## 24. 子组件可以直接改变父组件的数据吗？

**记忆口诀：「单向数据流，子改 prop 会警告，$emit 让父自己改」**
...
**记忆口诀：「render 触发 getter，Dep 收集 Watcher，setter notify 更新」**
...
### Vue3 中的依赖收集（Proxy + effect）

Vue3 使用 Proxy 代理对象代替 Object.defineProperty，依赖收集机制有以下变化：

| 对比项 | Vue2 | Vue3 |
| --- | --- | --- |
| 拦截方式 | `Object.defineProperty` | `Proxy` |
| 收集者 | `Dep` + `Watcher` | `ReactiveEffect` |
| 追踪粒度 | 需递归遍历所有属性 | 惰性代理，按需追踪 |
| 支持类型 | 仅普通对象/数组 | 支持 Map/Set/WeakMap/WeakSet |
| 数组处理 | 重写 7 个变异方法 | 原生拦截 |

**核心共通点**：两个版本都基于**发布-订阅模式**。每个响应式属性都是一个「发布者」，渲染函数和 watcher/effect 是「订阅者」，中间通过依赖收集器（Dep / ReactiveEffect）建立联系。

### 计算属性的依赖收集

计算属性通过**惰性求值 + 缓存**机制工作：

1. 计算属性被访问时，开始收集其内部依赖的响应式数据
2. 依赖数据变化时，不立即重新计算，而是标记 `dirty = true`
3. 下次访问时才重新求值，并缓存新结果
4. 若依赖未变化，直接返回缓存值

这种机制使计算属性在频繁读取但依赖变化少的场景下性能极优。


## 26. 对 React 和 Vue 的理解，它们的异同

**记忆口诀：「都用虚拟DOM和组件化；Vue 模板+双向绑定+自动优化，React JSX+单向数据流+手动优化」**
...
- **相似之处：**
  ...
  - **都支持native原生的方案，react的RN（reactNative）和vue的weex；**
...
**记忆口诀：「assets 走打包加哈希，static/public 原样拷贝不处理」**

**相同点：assets 和 static 两个都是存放静态资源文件。项目中所需要的资源文件图片，字体图标，样式文件等都可以放在这两个文件下，这是相同点**
**不相同点：assets 中存放的静态资源文件在项目打包时，也就是运行 npm run build 时会将 assets 中放置的静态资源文件进行打包上传，所谓打包简单点可以理解为压缩体积，代码格式化。而压缩后的静态资源文件最终也都会放置在 static 文件中跟着 index.html 一同上传至服务器。static 中放置的静态资源文件就不会要走打包压缩格式化等流程，而是直接进入打包好的目录，直接上传至服务器。因为避免了压缩直接进行上传，在打包时会提高一定的效率，但是 static 中的资源文件由于没有进行压缩等操作，所以文件的体积也就相对于 assets 中打包后的文件提交较大点。在服务器中就会占据更大的空间。**
**建议： 将项目中 template需要的样式文件js文件等都可以放置在 assets 中，走打包这一流程。减少体积。而项目中引入的第三方的资源文件如 iconfont.css 等文件可以放置在 static 中，因为这些引入的第三方文件已经经过处理，不再需要处理，直接上传。**

## 28. delete和Vue.delete删除数组的区别

**记忆口诀：「delete 置空不变 length，Vue.delete/splice 真删触发更新」**


| 操作 | 结果 | 数组长度 | 是否触发响应式 |
| --- | --- | --- | --- |
| `delete arr[1]` | `arr[1]` 变为 `empty/undefined`，其他元素索引不变 | 不变 | ❌ |
| `Vue.delete(arr, 1)` / `this.$delete(arr, 1)` | 直接删除元素，后续元素索引前移 | 减少 | ✅ |

```javascript
let arr = [1, 2, 3]
delete arr[1]        // [1, empty, 3]，length 仍为 3
Vue.delete(arr, 1)   // [1, 3]，length 变为 2，且视图更新
```

本质区别：`delete` 是 JS 原生操作符，只是把值置空而不改变数组结构；`Vue.delete` 内部调用了 `splice()` 并手动触发依赖通知，所以能正确更新视图。

## 29. vue如何监听对象或者数组某个属性的变化

**记忆口诀：「变异方法可监听，索引/新属性 Vue2 用 $set，Vue3 Proxy 全搞定」**
...
**记忆口诀：「parse → optimize → generate，产出 render 函数」**
...
## 31.  对keep-alive的理解，它是如何实现的，具体缓存的是什么？

**记忆口诀：「include/exclude 控缓存，max 限数量 LRU 淘汰，缓存 VNode + DOM 实例不销毁」**
...
## 32. Vue中封装的数组方法有哪些，其如何实现页面更新

**记忆口诀：「push/pop/shift/unshift/splice/sort/reverse 七个变异方法，执行后 dep.notify()」**
...
## 33. 说一下Vue的生命周期
**记忆口诀：「创挂更销」四阶段，每阶段 before + after**

| 阶段 | 钩子 | 能访问 data | 能访问 DOM | 常用操作 |
| --- | --- | :---: | :---: | --- |
| 创建 | `beforeCreate` | ❌ | ❌ | 初始化插件 |
| 创建 | `created` | ✅ | ❌ | 发请求、初始化数据 |
| 挂载 | `beforeMount` | ✅ | ❌ | - |
| 挂载 | `mounted` | ✅ | ✅ | 操作 DOM、发请求 |
| 更新 | `beforeUpdate` | ✅（新值） | ✅（旧DOM） | - |
| 更新 | `updated` | ✅ | ✅（新DOM） | 避免在此改数据 |
| 销毁 | `beforeDestroy` | ✅ | ✅ | 清定时器、解绑事件 |
| 销毁 | `destroyed` | ❌ | ❌ | - |

> Vue3 对应变化：`beforeDestroy` → `beforeUnmount`，`destroyed` → `unmounted`，setup() 替代 `beforeCreate` + `created`

**Vue 实例有⼀个完整的⽣命周期，也就是从开始创建、初始化数据、编译模板、挂载Dom -> 渲染、更新 -> 渲染、卸载 等⼀系列过程，称这是Vue的⽣命周期。**
...
Vue3 新增了三个特殊钩子，这是与 Vue2 的重要差异：

| 钩子 | 触发时机 | 用途 |
| --- | --- | --- |
| `onRenderTracked` | 响应式依赖被**收集**时 | 调试：查看哪些数据被组件追踪 |
| `onRenderTriggered` | 响应式依赖被**触发**导致重渲染时 | 调试：定位是哪个数据变化导致了更新 |
| `onServerPrefetch` | 组件实例在服务器上被渲染**前** | SSR：服务端数据预取 |

> 前两个钩子**仅在开发模式下可用**，不会在生产环境和 SSR 中被调用。
...
export default {
  setup() {
    onRenderTracked(({ key, target, type }) => {
      console.log('依赖被收集:', { key, target, type })
      // type 为 "get"，表示属性被读取并收集为依赖
    })
    
    onRenderTriggered(({ key, target, type }) => {
      console.log('触发重渲染:', { key, target, type })
      // type 为 "set"，表示属性被修改触发更新
    })
  }
}
```
...
| 生命周期 Vue2 | 生命周期 Vue3 (组合式 API) | 描述 |
| --- | --- | --- |
| `beforeCreate` | `setup()` 替代 | 组件实例创建之初 |
| `created` | `setup()` 替代 | 组件实例创建完成 |
...
| - | `onRenderTracked` | 调试：依赖被收集 |
| - | `onRenderTriggered` | 调试：依赖被触发 |
| - | `onServerPrefetch` | SSR 数据预取 |

### 常见生命周期应用场景

| 钩子 | 适用场景 |
| --- | --- |
| `beforeCreate` / `setup()` | 插件初始化、依赖注入 |
| `created` / `setup()` | 发起 API 请求、初始化数据 |
| `mounted` | 操作 DOM、初始化第三方库、获取元素尺寸 |
| `beforeUpdate` | 获取更新前的状态快照 |
| `updated` | DOM 已更新后的操作（避免在此改数据） |
| `beforeUnmount` | 清除定时器、取消订阅、解绑事件 |
| `unmounted` | 清理与其他实例的连接 |


## 34. Vue 子组件和父组件执行顺序

**记忆口诀：「父 before → 子完整 → 父 after；创建从外到内，渲染从内到外」**
...
## 35. created和mounted的区别

**记忆口诀：「created 有数据没 DOM，mounted 数据 DOM 全都有」**

| 对比项 | created | mounted |
| --- | --- | --- |
| 调用时机 | 实例创建完成，模板渲染成 HTML **前** | 模板渲染成 HTML **后**，DOM 已挂载 |
| 能否访问 data | ✅ | ✅ |
| 能否访问 DOM ($el) | ❌ | ✅ |
| 常见用途 | 初始化数据、发起不依赖 DOM 的请求 | 操作 DOM、初始化第三方库（如图表、地图） |

- **created**：适合做数据初始化和不依赖 DOM 的异步请求，是最早可以访问 `data` 和 `methods` 的钩子
- **mounted**：适合需要操作 DOM 的场景，此时 `this.$el` 已经替换为真实 DOM 节点

## 36. 一般在哪个生命周期请求异步数据

**记忆口诀：「不依赖 DOM 就 created，依赖 DOM 就 mounted，SSR 只能 created」**
...
**记忆口诀：「切走 deactivated，切回 activated，不走 destroy，首次 mounted + activated」**
...
## 38. 讲一下v-if和v-for的优先级
**记忆口诀：「Vue2 for优先，Vue3 if优先」**

| 版本 | 优先级 | 同时使用的问题 |
| --- | --- | --- |
| Vue2 | `v-for` > `v-if` | v-if 在每次 v-for 循环中都会执行，性能浪费 |
| Vue3 | `v-if` > `v-for` | v-if 先执行时访问不到 v-for 的 item，会报错 |

**最佳实践（两个版本通用）：**

1. **用 computed 过滤数据**（推荐）：先用计算属性过滤掉不需要的项，再 v-for 渲染
2. **用 `<template>` 包裹**：`<template v-for="item in list">` 内部再用 `v-if`
...
**记忆口诀：「component: () => import() 按需分包，减少首屏体积」**
...
```
路由懒加载可以提升首屏性能
## 40. 路由的hash和history模式的区别

**记忆口诀：「hash 带 # 不发请求兼容好，history 无 # 需后端配合 pushState」**
...
打包发布：在使用webpack等工具打包发布时，需要配置正确的publicPath,保证HTML中引用的资源路径正确。同时需要注意，如果项目部署在子路径下（如 `/app/`），需要正确配置 `base`（Vue Router）和 `publicPath`（构建工具），确保静态资源路径正确。
...
| 对比项 | `$route` | `$router` |
| --- | --- | --- |
| 本质 | 路由信息对象（当前路由的快照） | 路由实例对象（Vue Router 的实例） |
| 包含内容 | `path`、`params`、`query`、`hash`、`fullPath`、`matched`、`name`、`meta` | `push()`、`replace()`、`go()`、`back()`、`forward()`、`beforeEach()` 等 |
| 可写性 | **只读**，不应直接修改 | 可调用方法进行导航 |
| 使用场景 | 获取当前路由参数、路径等信息 | 进行路由跳转、添加导航守卫 |

```javascript
// $route：读取路由信息
this.$route.params.id    // 获取动态路由参数
this.$route.query.search // 获取查询参数
...
| 方式 | 是否刷新页面 | 是否触发 Vue 生命周期 | 原理 |
| --- | :---: | :---: | --- |
| `location.href = '/url'` | ✅ 整页刷新 | ❌ 销毁重建 | 浏览器发起新的 HTTP 请求 |
| `history.pushState('/url')` | ❌ 不刷新 | ❌ 不感知 | 仅修改 URL 和历史记录 |
| `router.push('/url')` | ❌ 不刷新 | ✅ 正常触发 | 底层使用 pushState + Vue 响应式路由匹配 |

- `location.href`：简单粗暴，会重新加载整个页面，丢失 SPA 的状态
- `history.pushState`：只改 URL 不做任何 Vue 组件切换，需要自己处理视图更新
- `router.push`：Vue Router 封装了 pushState，在修改 URL 的同时匹配对应路由组件，执行导航守卫，触发组件生命周期，实现按需渲染

## 46. params和query的区别

**记忆口诀：「query 走 path 像 GET 显示在 URL，params 走 name 像 POST 刷新会丢」**

| 对比项 | query | params |
| --- | --- | --- |
| 引入方式 | 用 `path` 引入 | 用 `name` 引入 |
| URL 显示 | 显示在 URL 中（`?key=value`） | 不显示在 URL 中 |
| 类比 | 类似 HTTP GET 传参 | 类似 HTTP POST 传参 |
| 刷新页面 | **不丢失**数据 | **会丢失**数据（除非路由定义了 `:param`） |
| 接收方式 | `this.$route.query.key` | `this.$route.params.key` |
...
// URL: /user（参数不在 URL 中，刷新会丢失）

// 如果路由定义了动态段 path: '/user/:id'
// 则 params 方式 URL 为 /user/1，刷新不会丢失
```

## 47. Vue-router 导航守卫有哪些

**记忆口诀：「全局 beforeEach/afterEach，路由独享 beforeEnter，组件内 beforeRouteEnter/Update/Leave」**
...
Vuex的核心概念包括：state、mutations、actions和getters。.其中，state是应用的状态，而mutations用于修改state中的状态。actions则用于处理异步操作或批量的同步操作，最终通过mutations来改变state。.getters!则用于对state中的数据进行计算或过滤。
...
**通俗点理解就是，vuex 弱化 dispatch，通过commit进行 store状态的一次更变;简化了 action 概念（Vuex 中 action 用于异步操作，mutation 用于同步修改 state）;弱化reducer，基于commit参数直接对数据进行转变，使得框架更加简易;**
...
**于是在 Vue3 中，新增了 Composition API，与 Options API 并存，转而投向 Composition API。Composition API本质上是将 Options API 背后的机制暴露给用户直接使用，这样用户就拥有了更多的灵活性，也使得 Vue3 更适合于 TypeScript 结合。**
...
看一下页面渲染的流程：解析HTML -> 生成DOM->生成 CSSOM->Layout->Paint->Composite
...
当一个组件第一次被挂载时，mounted 生命周期会被触发，同时 keep-alive 中的缓存组件还没有被渲染，但实际上，keep-alive 包裹的组件**首次挂载时 activated 也会触发**（在 mounted 之后）。再次从缓存中激活时，不走 mounted，只触发 activated。因此正确的触发顺序是：首次进入 mounted → activated，再次进入只有 activated，离开触发 deactivated。
...
| 方式 | 适用关系 | 说明 |
| --- | --- | --- |
| **props** | 父 → 子 | 最常用，单向数据流，支持类型校验和默认值 |
| **$emit** | 子 → 父 | 子组件触发自定义事件，父组件监听 |
| **$parent / $children** | 父子互访 | 直接访问实例，Vue3 已废弃 $children |
| **$refs** | 父 → 子 | 通过 ref 获取子组件实例，适合调用子组件方法 |
| **$attrs / $listeners** | 隔代传递 | 透传未声明的 props 和事件，Vue3 中 $listeners 已合并到 $attrs |
| **provide / inject** | 祖先 → 后代 | 跨层级注入，适合插件/主题等全局配置 |
| **$root** | 任意 → 根 | 访问根组件实例 |
| **EventBus** | 任意组件 | 事件总线，Vue3 需第三方库（mitt），适合简单场景 |
| **Vuex** | 全局状态 | Vue2 推荐，集中式状态管理 |
| **Pinia** | 全局状态 | Vue3 推荐，更轻量，支持 Composition API |

**按组件关系分类记忆：**

- **父子组件**：props / $emit / $parent / $refs / $attrs
- **兄弟组件**：$parent / $root / EventBus / Vuex / Pinia
- **跨层级关系**：EventBus / provide+inject / Vuex / Pinia

**Vue3 中的废弃与变化：**

- `$children` 已废弃，用 `$refs` + template refs 替代
- `$listeners` 已废弃，事件监听器合并到 `$attrs` 中
- `$on` / `$off` / `$once` 已废弃，EventBus 需使用第三方库（如 mitt）
- 推荐用 **Pinia** 替代 Vuex 进行全局状态管理

**$attrs 和 $listeners 的用法（解决隔代传递问题）：**

- `$attrs`：包含父作用域中不被 props 识别的属性绑定（class 和 style 除外），可通过 `v-bind="$attrs"` 传入内部组件，配合 `inheritAttrs: false` 使用
- `$listeners`（Vue2）：包含父作用域中的 v-on 事件监听器（不含 .native），可通过 `v-on="$listeners"` 传入内部组件

```html
<!-- 父组件 -->
<child-dom :foo="foo" :bar="bar" @upFoo="update"></child-dom>

<!-- 子组件：只声明 foo，bar 通过 $attrs 透传给孙组件 -->
<template>
  <div>
    <p>foo:{{ foo }}</p>
...
<template>
  <div>
    <p>bar:{{ bar }}</p>
    <button @click="$emit('upFoo', 'newFoo')">更新foo</button>
  </div>
</template>
<script>
export default {
...
虽然 EventBus 简单易用，但在中大型项目中有明显缺陷：

| 缺陷 | 说明 |
| --- | --- |
| **增加耦合性** | 组件需知道其他组件发布的事件名和数据结构 |
| **缺乏状态追踪** | 不像 Vuex/Pinia 有严格的 mutation 追踪机制，状态变更不可控 |
| **难以全局管理** | 适合简单场景，复杂应用中事件数量庞大时成为性能瓶颈 |
| **调试困难** | 事件来源和订阅者难以追溯，多人协作时维护性差 |

**隔代通信决策建议**：

- 2-3 层嵌套 → `$attrs` / `provide+inject`
- 全局共享状态 → Pinia（推荐）/ Vuex
- 简单的一次性事件 → EventBus / mitt
- 访问子组件实例 → `$refs` + template refs
...
2、Vue 3.x 使用惰性代理：只有在实际访问嵌套对象时才递归 reactive()，因此初始化开销更小，而 Vue 2.x 初始化时会递归地将嵌套对象转为响应式，但对于**新增属性**需要使用 `$set`。
...
事实上，通过索引访问或者修改数组中已经存在的元素，是可以触发 get 和 set 的，但是对于通过 push、unshift 增加的元素，会增加一个索引，这种情况需要手动初始化，新增加的元素才能被监听到。另外， 通过 pop 或 shift 删除元素，会删除并更新索引，也会触发 setter 和 getter 方法。
...
- 同级比较中为了减少元素移动次数，采用了双指针配合最长递增子序列算法
...
Vue 3 采用了结合编译期静态标记（patchFlag/dynamicChildren）的优化 Diff 算法。其主要原理如下：
...
- 对动态节点，即有可能发生变化的节点，Vue 3 采用了优化的Heuristic算法，通过比较新旧虚拟DOM节点的 type、key 和 patchFlag 信息，判断是否需要进行详细的Diff比较。

4、更新实际DOM：根据Patch的结果，Vue 3 将需要更新的部分进行批量更新，使实际DOM与新的虚拟DOM树保持一致。

下面，我们继续了解 Vue 是如何将两棵树的完全 DIFF 的时间复杂度从 O(n3) 降到 O(n) 的。

**通常而言两个树的完全对比 DIFF 比较算法：O(n3)**
...
这里的核心点在于，用传统算法对两棵树进行 DOM DIFF，时间复杂度太高，势必会导致更新效率低，因此，不管是 Vue 还是 React，都在寻求时间复杂度更低的 DOM DIFF 算法。
...
**4、**Vue 为了尽量不移动，先左右夹击跳过不变的，再找到最长递增子序列保持不动，移动其他元素。
...
| 分类 | 优化手段 |
| --- | --- |
| **页面渲染加载** | 避免所有数据都响应式、合理用 computed/watch、列表虚拟化、SSR |
| **页面更新** | v-once / v-memo、shallowRef/shallowReactive、v-for 加 key、合理用 v-if/v-show、keep-alive |
| **构建优化** | 路由懒加载、Tree-shaking、包体积分析、SourceMap 策略 |

### 页面渲染加载优化

**1. 避免将所有数据设置为响应式**

不需要响应式的数据不要放在 `data` 中（会增加 getter/setter 和 watcher 开销），可以直接定义在实例上：

```javascript
export default {
  data: () => ({}),
  beforeCreate() {
    this.timer = null  // 非响应式数据
  }
}
```
...
- 纯展示数据用 `Object.freeze()` 冻结，跳过响应式处理
- 大量数据使用**虚拟列表**（vue-virtual-scroller 等）只渲染可视区域

```javascript
async created() {
  const users = await axios.get("/api/users")
  this.users = Object.freeze(users) // 冻结后不会添加响应式
}
```

**3. SSR 服务端渲染**

优点：更快的首屏加载、更好的 SEO。缺点：服务端负载增大、部分浏览器 API 不可用、部署复杂度高。适合内容展示类网站，不适合内部管理系统。
...
- `v-memo="[deps]"`（Vue 3.2+）：依赖值不变时跳过子树更新，空数组 `v-memo="[]"` 等同于 v-once

**2. 减少大型不可变数据的响应性开销**

使用 `shallowRef()` / `shallowReactive()` 创建浅层响应式，只追踪顶层属性，深层对象需整体替换触发更新。

**3. v-for 加 key 且不与 v-if 同时使用**

- Vue2 中 v-for 优先级高于 v-if，会先循环再判断，浪费性能
- Vue3 中 v-if 优先级高于 v-for，v-if 中引用的循环变量不存在会报错
- 解决：用 computed 过滤列表，或将 v-if 放在外层包裹元素
...
**2. Tree-shaking 与包体积控制**

- 使用 ES Module 格式的依赖（如 lodash-es 替代 lodash）
- 模板预编译减少运行时编译器体积
- 用 bundlejs.com 等工具评估依赖体积
...
**优点**：
- **更快的首屏加载**：HTML 无需等待 JS 全部下载执行就能显示，在慢网速/低端设备上效果明显
- **更好的 SEO**：爬虫可直接看到完全渲染的页面
- **统一心智模型**：同一套 Vue 代码同时运行在服务端和客户端

**缺点**：
- 浏览器特定 API（如 `window`）只能在特定生命周期中使用
- 部署复杂度高，需要 Node.js 运行环境
- 服务端负载增大，高流量场景需缓存策略

> 判断标准：首屏加载速度对业务转化率至关重要 → 用 SSR；内部管理系统 → 不需要 SSR
...
### shallowRef / shallowReactive 使用示例

适用于处理超大型数组或深层嵌套对象（100,000+ 属性访问）：

```javascript
const shallowArray = shallowRef([/* 巨大的深层对象列表 */])
...
- 选择 ESM 格式依赖（如 `lodash-es` 替代 `lodash`）
- 使用 bundlejs.com 等工具评估依赖的实际增量
- 模板预编译（通过 Vue Loader）消除运行时编译器，减小体积
- SourceMap 在生产环境应关闭或单独存放，避免暴露源码且减少构建时间
...
| 层面 | 方式 | 说明 |
| --- | --- | --- |
| **逻辑扩展** | mixins、extends、Composition API | 共享/复用组件的逻辑代码 |
| **内容扩展** | 普通插槽、具名插槽、动态插槽、作用域插槽 | 让父组件自定义子组件的渲染内容 |

### 逻辑扩展方式对比

| 特性 | mixins | extends | Composition API |
| --- | --- | --- | --- |
| 数量 | 可混入多个 | 只能继承一个 | 可组合多个 |
| 冲突处理 | 选项合并策略（钩子都调用，其他组件优先） | 与 mixins 冲突时 extends 优先 | 不存在命名冲突 |
| 来源追踪 | ❌ 难以追踪 | ❌ 难以追踪 | ✅ 明确的函数调用 |
| TypeScript | ❌ 支持差 | ❌ 支持差 | ✅ 完美支持 |
| Vue3 推荐度 | 不推荐 | 不推荐 | **推荐** |
...
> **为什么「组合优于继承」？** Composition API 通过函数组合实现逻辑复用，避免了 mixins 的命名冲突、来源不明确、隐式依赖等问题。
...
<!-- 父组件 -->
<child-component>
  <template #header><h3>头部内容</h3></template>
  <template #content><p>主要内容</p></template>
</child-component>
```
...
<!-- 父组件 -->
<child-component :slotName="dynamicSlotName">
